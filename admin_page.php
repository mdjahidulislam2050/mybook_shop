...
<section class="dashboard">
    ...
    <div class="box-container">
        ...
        <div class="box">            
            <?php 
                $total_pandings = 0;
                $select_panding = mysqli_query($conn, "SELECT total_price FROM `orders` WHERE payment_status = 'pending'") or die('query faild');       
                if(mysqli_num_rows($select_panding) > 0){
                    while($fetch_pendings = mysqli_fetch_assoc($select_panding)){
                        $total_price = $fetch_pendings['total_price'];
                        $total_pandings += $total_price;
                    };
                };
            ?> 
            <h3><?php echo $total_pandings; ?></h3>
            <p>total pendings</p>
        </div>
        ...
        <div class="box">            
            <?php 
                $total_completed = 0;
                $select_completed = mysqli_query($conn, "SELECT total_price FROM `orders` WHERE payment_status = 'completed'") or die('query faild');       
